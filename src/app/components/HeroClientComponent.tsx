...
export default function HeroClientComponent() {
    ...
    return (
        <>
            <nav className="sticky top-0 z-50 bg-white/70 dark:bg-black/70 backdrop-blur-lg shadow-lg border-b border-gray-200 dark:border-gray-800 px-4 py-3 flex items-center justify-between max-w-screen-xl mx-auto sm:px-8 md:space-x-6">
                ...
                <div className="hidden md:flex space-x-6">
                    {navigation.map((item, idx) => (
                        <Link key={idx} href={item.reference} className="text-gray-700 dark:text-gray-200 hover:text-indigo-600 dark:hover:text-indigo-400 font-medium transition-colors duration-200">
                            {item.title}
                        </Link>
                    ))}
                </div>
                <button className="text-gray-500 dark:text-gray-300 outline-none md:hidden" onClick={() => setState(!state)}>
                    {state ? (
                ...
                {state && (
                    <div className="absolute top-full left-0 w-full bg-white/90 dark:bg-black/90 backdrop-blur-lg shadow-lg border-b border-gray-200 dark:border-gray-800 flex flex-col items-center py-4 md:hidden">
                        {navigation.map((item, idx) => (
                            <Link key={idx} href={item.reference} className="text-gray-700 dark:text-gray-200 hover:text-indigo-600 dark:hover:text-indigo-400 font-medium py-2 transition-colors duration-200">
                                {item.title}
                            </Link>
                        ))}
                    </div>
                ...
                <div className="max-w-screen-xl mx-auto text-gray-600 gap-x-12 items-center justify-between overflow-hidden md:flex md:px-8">
                    ...
                    <div className="flex-none mt-14 md:mt-0 md:max-w-xl animate-fade-in delay-400">
                        <Image
                            ...
                            width={400}
                            height={300}
                            className="rounded-3xl shadow-xl border-4 border-indigo-200"
